...
categorisedSites <- function(production_data, scenario_name){
  ...
  SmallRestocker <- MediumRestocker <- LargeRestocker <- SmallTable <- MediumTable <- LargeTable <- SmallOngrower <- MediumOngrower <- LargeOngrower <-
    SmallHatchery <- LargeHatchery <- SmallFishery <- MediumFishery <- LargeRestocker <- Code <- AuthorisationAndRegistrationStatusCode <- Year <- AmountInUnits <-
    CodeDescription <- AmountInUnits <- Units <- CodeDescription2 <- AnnualtonneTable <- AnnualCountHatch <- SmallHatch <- LargeHatch <- AnnualTonnes <- AnnualCount <-
    LargeFishery <- Category <- NULL
  ...
  tonnes <- Table[, .(AnnualTonnes = sum(AmountInUnits)), by = .(Code, Year)]
  ...
  Count <- Hatchery[, .(AnnualCount = sum(AmountInUnits)), by = .(Code, Year)]
