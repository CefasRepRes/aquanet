...
stateCosts <- function(data,
  ...
  cull_state <- sim_no <- stage <- . <- aggregated2 <- number_of_months <- value_t <- max_no_manage_cost <-site_id <- NULL
  ...
  if (state == "no_manage") {
    ...
    ######################################################################
    # Catchment costs are applied per month. Cumulative costs e.g. cost for all sides of business.
    ...
    # multiply total 'number_of_months' by corresponding site_type monthly costs to get state_costs
    aggregated2 <- merge(aggregated, state_unit_cost, by.x='site_types', by.y = 'site_type', all.x = TRUE)

    state_costs <- aggregated2[,duration_cost:= number_of_months * farm_cost_per_unit]
    ...
    aggregated2 <- merge(aggregated, state_unit_cost, by.x='site_types', by.y = 'site_type', all.x = TRUE)
    state_costs <- aggregated2[,duration_cost:= total_duration * farm_cost_per_unit]
