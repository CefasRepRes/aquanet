runSimulations <- function(graph.contactp.objects,
  ...
  allruns <-
    foreach::foreach(batchNo = 1:n_cores, .combine = c) %dorng% aquanet::simulationCode(
      ...
      out_createContactProbabilityMatrix = graph.contactp.objects,
      out_createWithinCatchmentEdges = graph.withinCatchmentEdges.objects,
      out_createCatchmentToSiteMatrix = graph.catchment2Site.objects,
      out_list_createRiverDistanceProbabilityMatrix = graph.riverDistance.objects,
      out_createDistanceMatrix = graph.estimateSiteDistances.objects,
